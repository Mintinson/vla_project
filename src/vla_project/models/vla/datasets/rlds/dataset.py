...
from vla_project.models.vla.datasets.rlds import obs_transforms, traj_transforms
from vla_project.models.vla.datasets.rlds.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    """Load a specific RLDS dataset from storage and getting it into a standardized format.
    ...
    def restructure(traj):
        ...
        if not all(k in traj for k in REQUIRED_KEYS):
            msg = f"Trajectory is missing keys: {REQUIRED_KEYS - set(traj.keys())}. Did you write a `standardize_fn`?"
            raise ValueError(
                msg,
            )
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    """Apply common transforms that happen at a trajectory level.
...
def apply_frame_transforms(
    ...
    *,
    ...
    """Apply common transforms that happen at a frame level.
...
def make_single_dataset(
    ...
    *,
    ...
    """Create a single dataset from kwargs. Returns a dataset of trajectories.
...
def make_interleaved_dataset(
    ...
    *,
    ...
    """Create an interleaved dataset from list of dataset configs (kwargs). Returns a dataset of batched frames.
    ...
    if len(sample_weights) != len(dataset_kwargs_list):
        msg = f"sample_weights must be None or have length {len(dataset_kwargs_list)}."
    ...
    if (traj_transform_kwargs is None) or (frame_transform_kwargs is None):
        msg = "Missing `traj_transform_kwargs` and `frame_transform_kwargs`!"
        raise ValueError(msg)
