...
def to_padding(tensor: torch.Tensor) -> torch.Tensor:
    """Generate padding tensor for the given tensor.

    Args:
        tensor: Input tensor to generate padding for.

    Returns:
        Padding tensor with appropriate zero values based on tensor dtype.

    Raises:
        ValueError: If tensor dtype is not supported for padding generation.
    """
    ...
    # For string tensors, PyTorch doesn't have native string support like TensorFlow
    # This would need to be handled differently depending on the specific use case
...
def normalize_action_and_proprio(traj: dict, metadata: dict, normalization_type: NormalizationType):
    ...
    match normalization_type:
        case NormalizationType.NORMAL:
            for key, traj_key in keys_to_normalize.items():
                mask = metadata[key].get("mask", torch.ones_like(metadata[key]["mean"], dtype=torch.bool))
                traj = dl.transforms.selective_tree_map(
                    traj,
                    match=lambda k, _: k == traj_key,
                    map_fn=lambda x: torch.where(mask, (x - metadata[key]["mean"]) / (metadata[key]["std"] + 1e-8), x),
                )
            return traj

        case NormalizationType.BOUNDS | NormalizationType.BOUNDS_Q99:
            for key, traj_key in keys_to_normalize.items():
                ...
                mask = metadata[key].get("mask", torch.ones_like(metadata[key]["min"], dtype=torch.bool))
                traj = dl.transforms.selective_tree_map(
                    traj,
                    match=lambda k, _: k == traj_key,
                    map_fn=lambda x: torch.where(
                        mask,
                        torch.clamp(2 * (x - low) / (high - low + 1e-8) - 1, -1, 1),
                        x,
                    ),
                )
                ...
                traj = dl.transforms.selective_tree_map(
                    traj,
                    match=lambda k, _: k == traj_key,
                    map_fn=lambda x: torch.where(zeros_mask, 0.0, x),
                )
            return traj

    msg = f"Unknown Normalization Type {normalization_type}"
    raise ValueError(msg)


def binarize_gripper_actions(actions: torch.Tensor) -> torch.Tensor:
    ...
    in_between_mask = torch.logical_not(input=torch.logical_or(open_mask, closed_mask))
    is_open_float = open_mask.to(torch.float32)
    ...
    # PyTorch implementation of the scan operation
    # Initialize result tensor
    result = torch.zeros_like(actions)
    carry = actions[-1].float()

    # Iterate in reverse order (equivalent to tf.scan with reverse=True)
    for i in reversed(range(actions.shape[0])):
        if in_between_mask[i]:
            result[i] = carry
        else:
...
def invert_gripper_actions(actions: torch.Tensor) -> torch.Tensor:
    return 1 - actions


def rel2abs_gripper_actions(actions: torch.Tensor) -> torch.Tensor:
    ...
    # PyTorch implementation of the scan operation for gripper state inversion
    # If no relative grasp, assumes open for whole trajectory
    nonzero_indices = (thresholded_actions != 0).nonzero(as_tuple=True)[0]
    if len(nonzero_indices) > 0:
        start = -1 * thresholded_actions[nonzero_indices[0]]
    else:
        start = torch.tensor(1, dtype=thresholded_actions.dtype, device=thresholded_actions.device)

    start = torch.where(start == 0, torch.tensor(1, dtype=start.dtype, device=start.device), start)

    # Initialize result tensor
    result = torch.zeros_like(actions)
    carry = start

    # Forward scan (equivalent to tf.scan without reverse=True)
    for i in range(actions.shape[0]):
        if thresholded_actions[i] == 0:
            result[i] = carry
        else:
            carry = thresholded_actions[i]
            result[i] = carry

    return result.float() / 2 + 0.5
...
def relabel_bridge_actions(traj: dict[str, Any]) -> dict[str, Any]:
    ...
    # PyTorch implementation of tf.nest.map_structure
    def map_structure_truncate(x):
        if isinstance(x, torch.Tensor):
            return x[:-1]
        elif isinstance(x, dict):
            return {k: map_structure_truncate(v) for k, v in x.items()}
        elif isinstance(x, list):
            return [map_structure_truncate(item) for item in x]
        else:
            return x

    traj_truncated = map_structure_truncate(traj)
    traj_truncated["action"] = torch.cat([movement_actions, traj["action"][:-1, -1:]], dim=1)
...
def get_dataset_statistics(
    ...
    dataset = dataset.traj_map(
        lambda traj: {
            "action": traj["action"],
            "proprio": (
                traj["observation"]["proprio"] if "proprio" in traj["observation"] else torch.zeros_like(traj["action"])
            ),
        },
    )
