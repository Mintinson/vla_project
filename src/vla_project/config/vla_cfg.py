...
@dataclass
class ExpSigLIP224pxBridge(VLAConfig):
...
@dataclass
class ExpCogACTOXEMagicSoupPlusMinus(ExpSigLIP224pxBridge):
...
class VLARegistry(Enum):
    ...
    SIGLIP_224PX_MX_BRIDGE = ExpSigLIP224pxBridge

    # === CogACT-VLA Pretraining Configs ===
    EXP_COGACT_OXE_MAGIC_SOUP_PLUS_MINUS = ExpCogACTOXEMagicSoupPlusMinus
...
if __name__ == "__main__":
    import draccus
    # draccus.parse() 会读取 dataclass，并从命令行解析参数
    # 它返回一个 TrainingConfig 的实例
    config = draccus.parse(VLAConfig)
